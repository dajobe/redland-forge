...
class AutoExitManager:
    ...
    def __init__(self, exit_delay_seconds: int = 300, enabled: bool = True):
        ...
        logging.debug(
            f"AutoExitManager initialized with {exit_delay_seconds}s delay, enabled={enabled}"
        )
    ...
    def on_build_completed(self, host_name: str, success: bool) -> None:
        ...
        logging.debug(
            f"Build completed for {host_name} (success={success}), scheduling auto-exit"
        )
        self._schedule_exit()
    ...
    def _schedule_exit(self) -> None:
        ...
        self.exit_timer = threading.Timer(self.exit_delay_seconds, self._perform_exit)
        self.exit_timer.start()
        logging.info(
            f"Auto-exit countdown started: {self.exit_delay_seconds} seconds remaining"
        )
